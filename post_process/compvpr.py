...
def draw_double_log_fit_in_diff_files(filenames, columns_to_fit, rent_length=None, savefigure=True):
    ...
    pics_directory = os.path.join(main_directory, 'pics')
    ...
    for i, column in enumerate(columns_to_fit):
        ...
        for j, filename in enumerate(sorted_filenames):
            ...
            # Apply logarithm to the y-axis data and handle invalid values
            valid_mask = (clean_data_value > 0) & (~np.isinf(clean_data_value))
            clean_data_value = clean_data_value[valid_mask]
            rent_exp_data = rent_exp_data[valid_mask]

            log_clean_data_value = np.log(np.log(clean_data_value))
            valid_mask = ~np.isinf(log_clean_data_value) & ~np.isnan(log_clean_data_value)
            log_clean_data_value = log_clean_data_value[valid_mask]
            rent_exp_data = rent_exp_data[valid_mask]

            if len(log_clean_data_value) == 0:
                print(f"Warning: All data for column '{column}' in file '{filename}' resulted in invalid log values.")
                continue
            label = f'Blocks: ${number}$'
            axs[i].scatter(rent_exp_data, log_clean_data_value, color=cmap(j), label=label)
            try:
                if rent_length is not None:
                    popt, _ = curve_fit(linear_func, rent_exp_data[:rent_length],
                                        log_clean_data_value[:rent_length])
                    fitted_values = linear_func(rent_exp_data[:rent_length], *popt)
                else:
                    popt, _ = curve_fit(linear_func, rent_exp_data, log_clean_data_value)
                    fitted_values = linear_func(rent_exp_data, *popt)

                fitted_values_region.append(fitted_values)

                a, b = popt
                a_sci = f"{a:.2e}".split('e')  # Split into base and exponent
                a_base, a_exp = a_sci[0], a_sci[1]
                # label = f'Blocks: ${number}$\n Fit: $\\ln(\\ln(y)) =  ({a_base} \\times 10^{{{a_exp}}})x + {b:.2f}$'
                #
                ...
                print(f"Error: Unable to fit linear curve for column '{column}' in file '{filename}'")
                continue
        ...
        y_label = y_axis_labels.get(column, f'Log(Log({column}))')
        axs[i].set_xlabel('Rent Exponent')
        axs[i].set_ylabel(f'Log(Log({column}))')
        axs[i].set_title(f'Double Logarithmic scale for {column}')
        axs[i].legend()
        axs[i].grid(True)

        individual_plot_path = os.path.join(pics_directory, f'{column}_vpr_flow_log_log.png')
        fig_individual, ax_individual = plt.subplots()
        ax_individual.scatter(rent_exp_data, log_clean_data_value, color=cmap(i), label=label)
        # if fitted_values_region:
        #     ax_individual.plot(rent_exp_data, fitted_values, color=color, linewidth=3, alpha=0.7)
        ax_individual.set_xlabel('Rent Exponent')
        ax_individual.set_ylabel(y_label)
        ax_individual.set_title(f'Double Logarithmic scale for {column}')
    ...
    if savefigure:
        ...
        plt.savefig(os.path.join(pics_directory, 'rent_exp_influence_vpr_flow_log_log.png'))
    plt.show()
...
def draw_log_fit_in_diff_files(filenames, columns_to_fit, rent_length=None, savefigure=True):
    ...
    for i, column in enumerate(columns_to_fit):
        ...
        for j, filename in enumerate(sorted_filenames):
            ...
            # Apply logarithm to the y-axis data
            log_clean_data_value = np.log(clean_data_value)

            axs[i].scatter(rent_exp_data, log_clean_data_value, color=cmap(j))
            try:
                if rent_length is not None:
                    ...
                    popt, _ = curve_fit(exponential_func, clean_data['rent_exp'], log_clean_data_value)
                    fitted_values = exponential_func(clean_data['rent_exp'], *popt)
        ...
        axs[i].set_title(f'Logarithmic Fit for {column}')
        axs[i].legend()
        axs[i].grid(True)
        ...
        ax_individual.scatter(rent_exp_data, log_clean_data_value, color=cmap(i), label=label)
        if fitted_values_region:
            ax_individual.plot(rent_exp_data, fitted_values, color=cmap(i), linewidth=3, alpha=0.7)
        ax_individual.set_xlabel('Rent Exponent')
        ax_individual.set_ylabel(y_label)
        ax_individual.set_title(f'Logarithmic scale for {column}')
        ax_individual.legend()
        ax_individual.grid(True)
        if savefigure:
            fig_individual.savefig(individual_plot_path)
    ...
    plt.tight_layout()
    if savefigure:
        plt.savefig(os.path.join(pics_directory, 'rent_exp_influence_vpr_flow_log.png'))
    plt.show()
...
def draw_fit_in_diff_files(filenames, columns_to_fit, rent_length=None, savefigure=True):
    ...
    for i, column in enumerate(columns_to_fit):
        ...
        fig_individual, ax_individual = plt.subplots()
        ax_individual.scatter(rent_exp_data, clean_data_value, color=cmap(i), label=label)
        if fitted_values_region:
            ax_individual.plot(rent_exp_data, fitted_values, color=cmap(i), linewidth=3, alpha=0.7)
        ax_individual.set_xlabel('Rent Exponent')
        ax_individual.set_ylabel(y_label)
    ...
    plt.tight_layout()
    if savefigure:
        plt.savefig(os.path.join(pics_directory, 'rent_exp_influence2vpr_flow.png'))
    plt.show()
...
if __name__ == '__main__':
    ...
    draw_double_log_fit_in_diff_files(csv_files,
                           columns_to_fit=['time', 'cpd', 'total_wirelength',
                                           'packing_time', 'routing_time', 'placement_time'], rent_length=33)